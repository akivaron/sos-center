...
import type { Incident, IncidentType, User } from "../types";
...
export function MapScreen({ copy, user, network, incidents, onIncidentsChange }: {
  ...
  const submitReport = async (incidentType: IncidentType, description: string) => {
  ...
  return (
    <View style={styles.screen} testID="map-screen">
      ...
      <BlurView intensity={82} tint="light" style={[styles.header, { top: insets.top + 8 }]} testID="network-status-header">
      ...
      <ReportSheet ref={reportRef} copy={copy} loading={submitting} onSubmit={submitReport} />
...
const styles = StyleSheet.create({
  ...
  header: { position: "absolute", left: 16, right: 16, height: 66, borderRadius: 22, overflow: "hidden", borderWidth: 1, borderColor: "rgba(255,255,255,0.8)", flexDirection: "row", alignItems: "center", paddingHorizontal: 16, ...shadow },
  statusDot: { width: 10, height: 10, borderRadius: 5, marginRight: 11 },
  headerText: { flex: 1 },
  brand: { fontSize: 17, fontWeight: "900", color: colors.ink, letterSpacing: -0.3 },
  network: { fontSize: 12, fontWeight: "700", color: colors.inkSoft, marginTop: 1 },
  alertCount: { minWidth: 46, height: 38, borderRadius: 14, backgroundColor: "rgba(244,244,245,0.8)", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 4 },
  alertCountText: { color: colors.ink, fontSize: 13, fontWeight: "900" },
  safeChip: { position: "absolute", alignSelf: "center", flexDirection: "row", alignItems: "center", gap: 7, borderRadius: 99, backgroundColor: "rgba(255,255,255,0.92)", paddingHorizontal: 13, height: 38, ...shadow },
  safeText: { color: colors.ink, fontSize: 12, fontWeight: "800" },
  locate: { position: "absolute", right: 16, width: 48, height: 48, borderRadius: 17, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center", ...shadow },
  reportButton: { position: "absolute", right: 20, minHeight: 50, borderRadius: 18, backgroundColor: colors.dark, flexDirection: "row", alignItems: "center", gap: 7, paddingHorizontal: 16, ...shadow },
  reportText: { color: "#FFFFFF", fontSize: 14, fontWeight: "900" },
  ...
  incidentCard: { position: "absolute", left: 16, right: 16, minHeight: 78, borderRadius: 20, backgroundColor: "#FFFFFF", flexDirection: "row", alignItems: "center", padding: 14, gap: 12, ...shadow },
  incidentIcon: { width: 44, height: 44, borderRadius: 15, backgroundColor: colors.brand, alignItems: "center", justifyContent: "center" },
  incidentText: { flex: 1 },
  incidentTitle: { fontSize: 16, fontWeight: "900", color: colors.ink },
